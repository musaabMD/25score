...
const Loading = () => (
  <div className="animate-pulse">
    <div className="h-24 bg-gray-200 rounded-md mb-4" />
  </div>
);
...
const QuizQuestion = ({ question }) => {
  ...
  return (
    <div className="relative bg-white rounded-lg overflow-hidden">
      ...
      <div className="px-4 space-y-3 mb-20">
        {question.choices.map((choice) => (
          ...
          >
            ...
            {showAnswers && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-500">{choice.percentage}%</span>
                {choice.correct && <Check className="w-5 h-5 text-green-500" />}
                {choice.id === selectedAnswer && !choice.correct && 
                  <X className="w-5 h-5 text-red-500" />
                }
      ...
      <Sheet open={activeSheet !== null} onOpenChange={closeSheet}>
        <SheetContent className="w-full sm:max-w-md">
